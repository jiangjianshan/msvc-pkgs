...
DEPENDS='freetype glib libffi libpng pixman pkg-config'
FWD=`dirname $(pwd)`
CWD=$(cd `dirname $0`; pwd)
PKG_NAME=cairo
PKG_VER=1.18.0
PKG_URL=https://www.cairographics.org/releases/$PKG_NAME-$PKG_VER.tar.xz
PKGS_DIR=$FWD/pkgs
...
configure_package()
{
  ...
  export PKG_CONFIG_PATH="$PKG_CONFIG_PATH_M"
  cd $SRC_DIR
  ...
  meson setup $BUILD_DIR_M                                                                     \
    ...
    -Dc_std=c17                                                                                \
    -Dc_args="$CFLAGS $CPPFLAGS_M"                                                             \
    -Dc_link_args="$(echo ${LDFLAGS_M//-L/\/LIBPATH:} | sed -e 's| /LIBPATH|,/LIBPATH|g')"     \
    -Dcpp_std=c++17                                                                            \
    -Dcpp_args="$CXXFLAGS $CPPFLAGS_M"                                                         \
    -Dcpp_link_args="$(echo ${LDFLAGS_M//-L/\/LIBPATH:} | sed -e 's| /LIBPATH|,/LIBPATH|g')"   \
    -Dc_winlibs="Ole32.lib,User32.lib"                                                         \
    -Dcpp_winlibs="Ole32.lib,User32.lib"                                                       \
    -Dtests=disabled > >(build_log) 2>&1 || print_error
}
