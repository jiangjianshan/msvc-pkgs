...
PKG_NAME=zstd
PKG_VER=1.5.6
PKG_URL=https://github.com/facebook/zstd/archive/refs/tags/v$PKG_VER.tar.gz
PKGS_DIR=$FWD/pkgs
...
patch_package()
{
  ...
  pushd $SRC_DIR
  patch -Np1 -i $PATCHES_DIR/001-zstd-1.5.6-fix-zstd-h-can-not-found.diff
  popd
}
...
configure_package()
{
  ...
  DEFINES="-DWIN32 -D_WIN32_WINNT=$WIN32_TARGET -D_CRT_DECLARE_NONSTDC_NAMES -D_CRT_NONSTDC_NO_WARNINGS -D_CRT_SECURE_NO_DEPRECATE"
  # NOTE:
  ...
  cmake -G "Ninja"                                                                                             \
    ...
    $SRC_DIR_M/build/cmake > >(build_log) 2>&1 || print_error
}
