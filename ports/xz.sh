...
PKG_NAME=xz
PKG_VER=5.4.6
PKG_URL=https://github.com/tukaani-project/xz/archive/refs/tags/v$PKG_VER.tar.gz
PKGS_DIR=$FWD/pkgs
...
patch_package()
{
  ...
  pushd $SRC_DIR
  patch -Np1 -i $PATCHES_DIR/001-xz-5.4.6-fix-no-package-liblzma-found-when-import.diff
  popd
}
...
install_package()
{
  ...
  cp -v $PREFIX_M/lib/liblzma.lib $PREFIX_M/lib/lzma.lib
  clean_build
  build_ok
}
