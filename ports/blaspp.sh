...
PKG_NAME=blaspp
PKG_VER=2024.05.31
PKG_URL=https://github.com/icl-utk-edu/blaspp/archive/refs/tags/v$PKG_VER.tar.gz
PKGS_DIR=$FWD/pkgs
...
patch_package()
{
  ...
  pushd $SRC_DIR
  patch -Np1 -i $PATCHES_DIR/001-blaspp-2024.05.31-fix-cmake-install-configdir-on-cygwin.diff
  popd
}
...
configure_package()
{
  ...
  check_triplet
  cd $BUILD_DIR
  OPENBLAS_PREFIX=`grep -E "\w+\s+openblas\s+.+" $OK_FILE | awk '{print substr($0, index($0, $4))}' | awk '{$1=$1};1'`
  # NOTE:
  ...
  # 3. See https://devblogs.microsoft.com/cppblog/improved-openmp-support-for-cpp-in-visual-studio/#new-openmpllvm-switch
  cmake -G "Ninja" \
    -DBUILD_SHARED_LIBS=ON                                                                 \
    -Dblas=OpenBLAS                                                                        \
    -DBLAS_LIBRARIES="$OPENBLAS_PREFIX/lib/openblas.lib"                                   \
    -Dbuild_tests=OFF                                                                      \
