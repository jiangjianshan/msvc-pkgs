...
DEPENDS='sqlite3 libtiff curl googletest'
FWD=`dirname $(pwd)`
CWD=$(cd `dirname $0`; pwd)
PKG_NAME=proj
PKG_VER=9.4.0
PKG_URL=https://download.osgeo.org/proj/$PKG_NAME-$PKG_VER.tar.gz
PKGS_DIR=$FWD/pkgs
...
patch_package()
{
  ...
  pushd $SRC_DIR
  patch -Np1 -i $PATCHES_DIR/001-proj-9.4.0-fix-error-c2491.diff
  popd
}
