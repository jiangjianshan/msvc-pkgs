...
PKG_NAME=doxygen
PKG_VER=1.11.0
PKG_URL=https://github.com/doxygen/doxygen/archive/refs/tags/Release_${PKG_VER//./_}.tar.gz
PKGS_DIR=$FWD/pkgs
...
prepare_package()
{
  ...
  if [ ! -d "$SRC_DIR" ]; then
    ...
    mv $PKG_NAME-Release_${PKG_VER//./_} $PKG_NAME-$PKG_VER
  fi
...
configure_package()
{
  ...
  check_triplet
  cd $BUILD_DIR
  LIBICONV_PREFIX=`grep -E "\w+\s+libiconv\s+.+" $OK_FILE | awk '{print substr($0, index($0, $4))}' | awk '{$1=$1};1'`
  # NOTE:
