...
DEPENDS='hwloc oneTBB level-zero'
FWD=`dirname $(pwd)`
CWD=$(cd `dirname $0`; pwd)
PKG_NAME=unified-memory-framework
PKG_VER=git
PKG_URL=https://github.com/oneapi-src/$PKG_NAME.git
PKGS_DIR=$FWD/pkgs
...
prepare_package()
{
  ...
  cd $PKGS_DIR
  if [ ! -d "$SRC_DIR" ]; then
    git clone --depth 1 --single-branch -b main $PKG_URL || print_error
  else
    pushd $SRC_DIR
