...
configure_package()
{
  ...
  check_triplet
  cd $BUILD_DIR
  OPTIONS='-MD -diagnostics:column -wd4819 -openmp:llvm'
  DEFINES="-DWIN32 -D_WIN32_WINNT=$WIN32_TARGET -D_CRT_DECLARE_NONSTDC_NAMES -D_CRT_SECURE_NO_WARNINGS -D_CRT_NONSTDC_NO_WARNINGS -D_CRT_SECURE_NO_DEPRECATE -D_SILENCE_CXX23_DENORM_DEPRECATION_WARNING"
  # NOTE:
...
install_package()
{
  ...
  cp -rv $PREFIX/docs/* $PREFIX/share/doc/$PKG_NAME
  rm -rfv $PREFIX/docs
  cp -rv $PREFIX/lib/x86_64/*.lib $PREFIX/lib
  rm -rfv $PREFIX/lib/x86_64
  mv $PREFIX/LICENSE $PREFIX/share/doc/filament/LICENSE
  mv $PREFIX/README.md $PREFIX/share/doc/filament/README.md
  clean_build
  build_ok
}
