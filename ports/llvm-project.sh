...
configure_package()
{
  ...
  check_triplet
  cd $BUILD_DIR
  bootstrap_cmake_options='CMAKE_BUILD_TYPE;CMAKE_INSTALL_PREFIX;CMAKE_PREFIX_PATH;CMAKE_POLICY_DEFAULT_CMP0076;CMAKE_POLICY_DEFAULT_CMP0114;CLANG_DEFAULT_CXX_STDLIB;CLANG_DEFAULT_RTLIB;CLANG_INCLUDE_DOCS;LLVM_BUILD_DOCS;LLVM_BUILD_EXAMPLES;LLVM_BUILD_TESTS;LLVM_BUILD_TOOLS;LLVM_BUILD_UTILS;LLVM_BUILD_LLVM_C_DYLIB;LLVM_ENABLE_ASSERTIONS;LLVM_ENABLE_DOXYGEN;LLVM_ENABLE_OCAMLDOC;LLVM_INCLUDE_DOCS;LLVM_INCLUDE_EXAMPLES;LLVM_INCLUDE_TESTS;LLVM_INCLUDE_TOOLS;LLVM_INCLUDE_UTILS;LLVM_INSTALL_UTILS;LLVM_ENABLE_PROJECTS;LLVM_ENABLE_RUNTIMES;LLVM_ENABLE_RTTI;LLVM_RUNTIME_TARGETS;LLVM_OPTIMIZED_TABLEGEN;LLVM_TARGETS_TO_BUILD;LIBCXX_USE_COMPILER_RT;LIBCXX_ENABLE_FILESYSTEM;Python3_EXECUTABLE'
  ...
  CPPFLAGS_M="$CPPFLAGS_M -D_SILENCE_NONFLOATING_COMPLEX_DEPRECATION_WARNING"
  # NOTE:
  ...
  cmake -G "Ninja"                                                                       \
    -DBOOTSTRAP_CMAKE_CXX_FLAGS="$bootstrap_cmake_cxx_flags $CXXFLAGS $CPPFLAGS_M"       \
    -DCMAKE_BUILD_TYPE=Release                                                           \
    -DCMAKE_CXX_FLAGS="$CXXFLAGS $CPPFLAGS_M"                                            \
    ...
    -DLLVM_ENABLE_PROJECTS="bolt;clang;clang-tools-extra;libc;lld;mlir;pstl;polly;flang" \
    -DLLVM_ENABLE_RUNTIMES="compiler-rt;libcxx;openmp"                                   \
    -DLLVM_ENABLE_RTTI=ON                                                                \
    -DLLVM_RUNTIME_TARGETS="$LLVM_TRIPLET"                                               \
