...
patch_package()
{
  ...
  popd
  echo [$0] Patching ltmain.sh in libcharset/build-aux
  ...
  echo [$0] Patching configure in libcharset
  pushd libcharset
  sed                                                                                                \
    -e "s|library_names_spec='\$libname.dll.lib'|library_names_spec='\$libname.lib'|g"               \
    -e 's|$tool_output_objdir$libname.dll.lib|$tool_output_objdir$libname.lib|g'                     \
...
configure_package()
{
  ...
  # Patch Makefiles
  echo [$0] Patching Makefile in src
  pushd src
  sed                                                                \
  ...
  echo [$0] Patching Makefile in srclib
  pushd srclib
  sed                                                                \
    -e "s|libicrt.a|libicrt.lib|g"                                   \
    Makefile > Makefile-t
  ...
  echo [$0] Patching Makefile in tests
  pushd tests
  sed                                                                \
    -e "s|libicrt.a|libicrt.lib|g"                                   \
    Makefile > Makefile-t
  ...
  echo [$0] Patching Makefile in lib
  pushd lib
  sed                                                                \
