...
configure_stage()
{
  ...
  ./Configure --host="$HOST_TRIPLET"                                                     \
    ...
    --graphic=win32                                                                      \
    --time=ftime                                                                         \
    --with-runtime-perl="$(cygpath -u "$PERL_PREFIX")/bin/perl.exe"                      \
    --with-readline="$(cygpath -u "$READLINE_PREFIX")"                                   \
    --with-ncurses-lib="$(cygpath -u "$NCURSES_PREFIX")/lib"                             \
    --with-gmp="$(cygpath -u "$GMP_PREFIX")"                                             \
    --with-fltk="$(cygpath -u "$FLTK_PREFIX")"
}
